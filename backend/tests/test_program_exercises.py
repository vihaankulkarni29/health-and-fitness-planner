...
def test_create_program_exercise(client: TestClient, db: Session, auth_headers: dict[str, str]) -> None:
...
def test_read_program_exercise(client: TestClient, db: Session) -> None:
    ...
    program_exercise = crud_program_exercise.create(db, obj_in=program_exercise_in)

    response = client.get(f"{settings.API_V1_STR}/program_exercises/{program_exercise.id}")
    assert response.status_code == 200
    content = response.json()
...
def test_read_program_exercises(client: TestClient, db: Session) -> None:
    response = client.get(f"{settings.API_V1_STR}/program_exercises/")
    assert response.status_code == 200
    content = response.json()
    assert isinstance(content, list)


def test_update_program_exercise(client: TestClient, db: Session, auth_headers: dict[str, str]) -> None:
...
def test_delete_program_exercise(client: TestClient, db: Session, auth_headers: dict[str, str]) -> None:
    ...
    response = client.delete(f"{settings.API_V1_STR}/program_exercises/{program_exercise.id}", headers=auth_headers)
    assert response.status_code == 200
    content = response.json()
    assert content["id"] == program_exercise.id
