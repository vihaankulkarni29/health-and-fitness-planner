...
def test_create_exercise_log(client: TestClient, db: Session, auth_headers: dict[str, str]) -> None:
    ...
    workout_session_in = {"trainee_id": trainee.id, "program_id": program.id, "session_date": str(date.today()), "status": "in-progress"}
    workout_session = crud_workout_session.create(db, obj_in=workout_session_in)
...
def test_read_exercise_log(client: TestClient, db: Session) -> None:
    ...
    exercise_log = crud_exercise_log.create(db, obj_in=exercise_log_in)
...
def test_read_exercise_logs(client: TestClient, db: Session) -> None:
...
def test_update_exercise_log(client: TestClient, db: Session, auth_headers: dict[str, str]) -> None:
...
def test_delete_exercise_log(client: TestClient, db: Session, auth_headers: dict[str, str]) -> None:
